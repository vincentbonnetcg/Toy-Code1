...
class AnchorSpring(ConstraintBase):
    ...
    def set_object(self, scene, node_id, kinematic, kinematic_parametric_point):
        ...
        x, v = na.node_state(scene.dynamics, node_id)
    ...
    def compute_forces(cls, datablock_cts : DataBlock, scene : Scene) -> None:
        ...
        for ct_index in range(len(datablock_cts)):
            ...
            x, v = na.node_state(scene.dynamics, node_ids[0])
...
class Spring(ConstraintBase):
    ...
    def set_object(self, scene, node_ids):
        ...
        element is an object of type self.datablock_ct generated in add_fields
        '''
        x0, v0 = na.node_state(scene.dynamics, node_ids[0])
        x1, v1 = na.node_state(scene.dynamics, node_ids[1])
    ...
    def compute_jacobians(cls, datablock_cts : DataBlock, scene : Scene) -> None:
        ...
        for ct_index in range(len(datablock_cts)):
            x0, v0 = na.node_state(scene.dynamics, node_ids_ptr[ct_index][0])
            x1, v1 = na.node_state(scene.dynamics, node_ids_ptr[ct_index][1])
